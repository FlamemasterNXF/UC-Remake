...
function buyCycle(i){
    if(data.particles[1].gte(CYCLES[i].cost())){
        ...
        data.cycleLevels[i-1] = data.cycleLevels[i-1].plus(1)
    }
}
...
function buyMaxCycles(){
  for(let i=1;i<=9;i++){
    ...
    if(i===10){
      ...
      let a = data.cycleLevels[0].div(10).add(1)
      let spent = startLevel.mul(startLevel.add(1)).div(2).add(a.mul(startLevel)).mul(i===1?5e4:1e5)
      let maxLevels = dp.div(11-i).div(i===1?5e4:1e5).add(spent.div(i===1?5e4:1e5)).mul(8).add(a.mul(2).add(1).pow(2)).sqrt().div(2).sub(a.add(0.5)).clampMin(startLevel).floor()
      // the 11-i is there to distribute dp evenly
      data.cycleLevels[i] = maxLevels
      let totalCost = maxLevels.mul(maxLevels.add(1)).div(2).add(a.mul(maxLevels)).mul(i===1?5e4:1e5).sub(spent)
      data.particles[1]=data.particles[1].sub(totalCost)
    }
  }
