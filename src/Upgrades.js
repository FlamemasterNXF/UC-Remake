...
function calculateUpgradeCosts(){
    for (let i=0;i<data.upgrades.length-1;i++){
        ...
        data.upgrades[i].c = data.upgrades[i].amt.gte(1)?
            costBases[i].times(D(1).plus(data.upgrades[i].amt.plus(1).div(1.1).pow(data.upgrades[i].amt))).div(theoryEffects[6]): costBases[i]
    }
...
function calculateUpgradeEffects(){
    ...
    upgradeEffects[4] = D(data.upgrades[4].amt.plus(1)).times(theoryEffects[8])
}