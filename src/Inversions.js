const INVERSIONS = {
    prevDeep: D(0),
    requirement(){
        return D(1e150).pow(this.deepInversionEffects()[2])
    },
    totalITheoryLevels(){
        let total = D(0)
    ...
    calcGain(){
        return data.oddities.gte(this.requirement()) ? ((D(data.oddities.exponent-this.requirement().exponent).plus((data.entropy.sqrt().div(100)).clampMin(1))).times(this.iTheoryEffects()[4])).pow(this.deepInversionEffects()[1]): D(0)
    },
    gain(diff){
    ...
    effectDescriptions: [
        'Inversions boost Breakpoint 3', 'Inversions boost Oddity gain while the Lost Derivative is active', 'Inversions Boost the Lost Theory of Cycles',
    ...
    updateHTML(){
        ...
        DOM('inversionInversion').innerHTML = data.inversionInversionControl[1]?`<i>Your Inversions are Inverted</i><br>Undo the Unthinkable things you've done`:data.inversionInversionControl[0]?`<i>Are you prepared to venture into the far unknown?</i><br>Invert your Inversions`:`Unknown<br>Requires: 3 Deep Inversions`
    ...
    iTheoryEffects() {
        ...
        iTheoryEffects[4] = data.invertedTheoryLevels[4].sub(data.invertedTheoryLevels[4].div(2)).plus((data.particles[0].log10().sub(data.invertedTheoryLevels[4].times(2))).div(10)).clampMin(1)
    ...
    controlInversionInversion(){
        if(!data.inversionInversionControl[0] && data.deepInversionCap.gte(3)){ data.inversionInversionControl[0] = true }
