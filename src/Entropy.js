const ENTROPY = {
    entropyGain() {
        ...
        if(data.oddities.gte(1e300) && cond){ return (data.oddities.sqrt()).pow(INVERSIONS.deepInversionEffects()[2]).times(INVERSIONS.inversionEffect().log2()) }
        if(data.oddities.gt(1e150) && cond ){ return (data.oddities.log10()).pow(INVERSIONS.deepInversionEffects()[2]).times(INVERSIONS.inversionEffect().log2()) }
